...
from AppBlog.forms import UserRegistrationForm, UserEditForm, AvatarForm
from AppBlog.models import *
from mensajeria.models import *
from django.views.generic import ListView
from AppBlog.forms import AvatarForm
...
def enviarMensaje(request, pk=None):
    ...
    if request.method == 'POST':
        try:
          ...
          return render (request, 'AppBlog/index.html', {'mensaje': 'Usuario no válido, intente nuevamente.', 'error':'error', 'url':avatar[0].avatar.url  })
        if destinatario:
            ...
            mensaje.save()
            return render (request, 'AppBlog/index.html', {'mensaje': 'Mensaje enviado.','success':'success', 'url':avatar[0].avatar.url})
        else:
            return render (request, 'AppBlog/index.html', {'mensaje': 'Usuario no válido, intente nuevamente.', 'error':'error', 'url':avatar[0].avatar.url  })
...
def nuevoAnuncio(request):
    ...
    if user.groups.filter(name='Usuarios_Comunes').exists():
        return render (request, 'AppBlog/index.html', {'mensaje': 'NO PERMITIDO.', 'error':'error', 'url':avatar[0].avatar.url })
    ...
    if request.method == 'POST':    
        ...
        try:
            ...
            return render (request, 'AppBlog/index.html', {'mensaje': 'Se ha producido un error.', 'error': 'error', 'url':avatar[0].avatar.url })   

        return render (request, 'AppBlog/index.html', {'mensaje': 'Anuncio simple creado exitosamente.', 'success':'success', 'url':avatar[0].avatar.url })    
